...
int traceLuggage(int& HP1, int& EXP1, int& M1, const int& E3){
	...
	if (k) M1 = ceil(M1-(k*E3)/9.0);
	else M1 -= E3*9;
	
	// RACE 2 
	int N[20] = {2, 3, 5, 7, 11, 13, 17};
	int N1[10] , N2[20] , s = 0  , m = 0 ;
	for (int i = 1 ; i < 8 ; i++){
    	N1[i] = (N[i-1]+E3)%26; 
    	s += N1[i]; 
    }
	...
	// RACE 3   (co TH ca 2 diem tren 1 duong)
	...
	for(int i = 1 ; i < 21 ; i++) 
	    D2[i] = ((D1[i]+E3)*(minn/minn_idx))%26 + 65;
	k = 0;    
	for(int i = 12 ; i > 0 ; i--){
		if (D2[i] == 80){
			k = 13 - i;
